...
class AliSms{
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = "LTAIWWetElWn9hy5"; // 开发AccessKeyId

        $accessKeySecret = "VwrIqo53NTmlk7uF3AzvtACTOWgY0K"; // 开发AccessKeySecret
     ...
     * $code 验证码
     */
    public static function sendSms($phone,$moduleCode,$code) {
        ...
        $request->setSignName("谷乐数据");

        // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
        $request->setTemplateCode($moduleCode);
...
//            "阿里云短信测试专用"
        ...
        // 必填:短信模板-可在短信控制台中找到
        $request->setTemplateCode("SMS_134135448");
...
//            ),
    ...
    /**
     * 说明: 发送短信验证码
     * @param not null $cType   类型：不同类型对应不同模板 1:注册 2：登录 3：忘记密码 5:微信绑定手机号
     * @param not null $saveKey redis保存的key,用途分类
     * @param not null $userTel 手机号
     * @return
     */
    public static function  sendSmsCode($cType,$saveKey,$userTel){
        $ALISMS_MODULES = Constants::ALISMS_MODULES;
        //code 验证码
        $code = static::getRandomNum(6);
        if($ALISMS_MODULES[$cType]){
            $ret = static::sendSms($userTel,$ALISMS_MODULES[$cType],$code);
            if ($ret->Code =="OK") {
                \Yii::redisSet("{$saveKey}:{$userTel}", $code, 600);
                return \Yii::jsonResult(600, '发送成功', $ret->Message);
            } elseif($ret->Code =="isv.BUSINESS_LIMIT_CONTROL"){
                return \Yii::jsonError(109, '操作频繁，请稍后再试');//使用同一个签名，对同一个手机号码发送短信验证码，支持1条/分钟，5条/小时 ，累计10条/天。
            }else{
                return \Yii::jsonError(109,$ret->Message);
            }
        }else{
            return \Yii::jsonError(109, '发送失败',"没有对应的模板类型");
        }
    }
    /**
     * 说明: 获取本地存储（redis）短信码
