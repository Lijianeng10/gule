...
require_once \Yii::$app->basePath . "/modules/components/wxpay/applib/log.php";
...
class wxpayapp {
    ...
    public function __construct() {
        $clog = new \CLogFileHandler(\Yii::$app->basePath . "/modules/components/log.txt");
        \Log::Init($clog);
        $this->notifyUrl = "http://" . \Yii::$app->params["userDomain"] . "/api/publicinterface/interface/notify_wxpayapp";
    }
...
//        $input->SetNotify_url('http://php.javaframework.cn/api/sports/superlotto/paixu');
//        $input->SetTrade_type("NATIVE");
//        $input->SetProduct_id($this->orderCode);
...
//    }
    ...
    public function productPrePay() {
        ...
        \Log::INFO("preOrder:" . json_encode($input->GetValues(), JSON_UNESCAPED_UNICODE));
        return $order;
    }
    ...
    public function refund($out_trade_no, $refund_fee, $total_fee, &$out_request_no) {
        if (isset($out_trade_no) && $out_trade_no != "") {
            ...
            \Log::INFO("refundOrder:" . json_encode($input->GetValues(), JSON_UNESCAPED_UNICODE));
