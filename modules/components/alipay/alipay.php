...
class alipay {
    ...
    public function Notify() {
        ...
        \Yii::$app->db->createCommand()->insert("logtest", ["text" => json_encode($post), "type" => "data-ali"])->execute();
        $arr = $post;
        if (isset($arr["s"])) {
            unset($arr["s"]);
        }
        ...
        if ($result) {//验证成功
            ...
            //商户订单号
            \Yii::$app->db->createCommand()->insert("logtest", ["text" => json_encode($post), "type" => "data-ali-success"])->execute();
...
//                }
            ...
            \Yii::$app->db->createCommand()->insert("logtest", ["text" => json_encode($post), "type" => "data-ali-fail"])->execute();
            //验证失败
            echo "fail";
        }
    }
...
//        var_dump($response);
    ...
    public function returnUrl() {
        ...
        \Yii::$app->db->createCommand()->insert("logtest", ["text" => json_encode($_SERVER), "type" => "data-ali-server"])->execute();
        \Yii::$app->db->createCommand()->insert("logtest", ["text" => json_encode($get), "type" => "data-ali-return"])->execute();
        $config = \config::Config;
        $alipaySevice = new \AlipayTradeService($config);
        $result = $alipaySevice->check($arr);
        $info = \app\modules\common\services\OrderService::getPayRecord($out_trade_no);
